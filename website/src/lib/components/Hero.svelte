<script lang="ts">
    ...
    onMount(() => {
        ...
        tl_img.to("#img", {rotationY: `+=${v}`, scale: 1.2, rotateX: `+=${v}`, duration: timing});
        tl_img.to("#img", {z: 50, duration: timing / 2, yoyo: true, repeat: 1}, 0);
...
<div class="min-h-[100vh] w-full hero-bg text-primary-foreground text-sm relative">
    <!-- Main content -->
    <div class="flex flex-col items-center justify-center text-center md:px-8 relative z-10">
        ...
        <h1 class="text-3xl md:text-4xl font-semibold leading-tight my-6 mx-3 max-w-4xl">
            <span bind:this={textElement}>Hi, I'm Durande Kamga,</span><br/>
            <span class="text-xl"> <WavyText text="Developer |"/> <WavyText text="Data scientist |"/> <WavyText
                    text="XAI Researcher"/></span>

        </h1>

        <p class="text-md leading-relaxed opacity-80 mb-12 max-w-2xl small-description">
            I build efficient backend systems and develop AI models with a focus on interpretability.<br/>
            Built fast, designed to convert, ready to grow.
        </p>

        <div class="model-3d hidden md:block absolute inset-0 z-50 top-1/3 left-3/4">
        ...
        <div class="z-20 mt-6 flex flex-wrap items-center justify-start gap-6 relative">
            <a class="group rounded-full m-1 p-1  hover:scale-150 transition-all hover:shadow-2xl hover:shadow-primary/50 hover:drop-shadow-xl/25 ease-in group-hover:fill-zinc-600 dark:fill-zinc-400 dark:group-hover:fill-zinc-300"
