    ...
    variants: {
       variant: {
          default: "bg-primary text-primary-foreground hover:bg-primary/90",
           black: "bg-slate-950 text-primary-foreground hover:bg-slate-950/90",
          destructive: "bg-destructive text-destructive-foreground hover:bg-destructive/90",
          outline: "border border-input bg-background hover:bg-muted-foreground hover:text-accent-foreground",
...
type Effect = VariantProps<typeof buttonVariants>["effect"]; // New: Define Effect type

// Define icon types for Svelte. You'll pass a component for the icon.
type IconPlacement = 'left' | 'right';

type Props = ButtonPrimitive.Props & {
    variant?: Variant;
    size?: Size;
    effect?: Effect; // New: Add effect prop
    icon?: ConstructorOfATypedSvelteComponent; // New: For Svelte components as icons
    iconPlacement?: IconPlacement; // New: For icon placement
};
