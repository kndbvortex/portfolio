<script lang="ts">
  ...
  const rgb = document.documentElement.classList.contains("dark")?hexToRgb('#ffff'):hexToRgb(color);
...
<div class={className} bind:this={canvasContainerRef} aria-hidden="true">
  <canvas bind:this={canvasRef} class="size-full dark:text-white"></canvas>
</div>
